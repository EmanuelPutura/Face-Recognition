...
class FaceRecognitionPipeline:
    def __init__(self, train_data_dir, validation_data_dir):
        ...
        self.__train_data = self.__datagen.flow_from_directory(train_data_dir, target_size=IMAGE_SHAPE[:2])
        self.__validation_data = self.__datagen.flow_from_directory(validation_data_dir, target_size=IMAGE_SHAPE[:2])

        self.__model = FaceRecognitionModel(self.__train_data.num_classes)
    ...
    def evaluate_model(self, test_data_dir):
        test_data = self.__datagen.flow_from_directory(test_data_dir, target_size=IMAGE_SHAPE[:2])
        return self.__model.evaluate_generator(test_data)
    ...
    def match_name(self, image):
        ...
        predicted_name = next((key for (key, value) in self.__train_data.class_indices.items() if value == name_index), None)
...
class FaceRecognitionModel(tf.keras.Sequential):
    def __init__(self, num_classes):
        ...
        self.add(keras.layers.Conv2D(32, (3, 3), activation='relu', input_shape=IMAGE_SHAPE))
